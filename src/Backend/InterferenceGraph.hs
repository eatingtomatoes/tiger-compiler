...
module Backend.InterferenceGraph
  ( InterferenceGraph
...
buildInterferenceGraph :: [Instruction Temp] -> InterferenceGraph
buildInterferenceGraph insts = uncurry (buildInterferenceGraph_ insts) $ calcInsOuts insts

buildInterferenceGraph_ :: [Instruction Temp] -> [Set.Set Temp] -> [Set.Set Temp] -> InterferenceGraph
...
coalesceTemp :: Int -> [Instruction Temp] -> InterferenceGraph -> ([(Temp, Temp)], InterferenceGraph)
coalesceTemp usableRegisters insts graph = (Map.toList $ foldr f mempty replacements, coalesced)
  where
    ...
    moveRelatedPairs = catMaybes $ fmap f' insts
      where
        f' Instruction{..} = case (_piOperator, _piAddressPattern) of
