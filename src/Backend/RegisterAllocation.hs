...
module Backend.RegisterAllocation
  ( RegisterDist
  , allocRegisters
  )where
...
import Backend.InterferenceGraph
import Graph
...
allocRegisters :: Monad m => Int -> [Instruction Temp] -> ExceptT String (StateT TempPool m) [Instruction Register]
...
allocRegisters' :: [Instruction Temp] -> Either Temp RegisterDist 
