...
module MiddleEnd.Tree.InstructionSelection
  ( selectFunHeader
  , selectFunBody
  , selectFunTail
  ) where
...
type SelectionContext m =
  ...
  , MonadWriter [Instruction Temp] m
  )

selectFunHeader :: Frame -> [Instruction Temp]
selectFunHeader Frame{..} = execWriter $ do
  emitPlain Push $ OnlyR rbp
  emitPlain Mov $ RR rbp rsp
  emit Instruction
    { _piOperator = Sub
    , _piAddressPattern = RC rsp (- _frPointer)
    , _piExtraDefs = []
    , _piExtraUses = []
...
selectFunTail :: [Instruction Temp]
...
-- selectFunBody :: InstSel m => [T.Stmt] -> m [Instruction Temp]
selectFunBody :: Monad m => [T.Stmt] -> ExceptT String (StateT TempPool m) [Instruction Temp]
selectFunBody body
  = withExceptT show
  $ execWriterT
  $ runReaderT (mapM_ codeStmt body) Nothing
        ...
        _ | elem op [T.Times, T.Divide] -> do
              ...
              case (e1, e2) of
                ...
                (e1', e2') -> do
                  ...
                  emitPlain pop $ OnlyR r2
                  emitPlain Mov $ RR temp rax
        _ -> error $ "unhandled binary operator in codeExpr: " <> show op
    T.UnOpExpr op e -> do
      let pop = case op of
            T.Not -> Not
            T.Negate -> Neg
      ...
      case (e1, e2) of
        (T.TempExpr r1, T.CallExpr (T.NameExpr label) args) -> do
          withArgs args $ \dsts srcs -> do
            emit Instruction
              { _piOperator = Call
              , _piAddressPattern = OnlyLabel label
              , _piExtraDefs = dsts
              , _piExtraUses = srcs
              , _piMark = Plain
              }
